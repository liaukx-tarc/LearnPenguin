...
class CourseCppFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        userDatabase.child("User").child(auth.uid.toString()).child("collection").child("1").get()
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.action_favourite -> {
                if(isFavorite)
                    userDatabase.child("User").child(auth.uid.toString()).child("collection").child("1").setValue(false)
                    ...
                    userDatabase.child("User").child(auth.uid.toString()).child("collection").child("1").setValue(true)
                        .addOnSuccessListener {
                            item.setIcon(R.drawable.star)
                            isFavorite = true
