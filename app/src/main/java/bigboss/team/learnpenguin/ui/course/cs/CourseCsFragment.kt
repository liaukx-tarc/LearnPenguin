...
class CourseCsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        userDatabase.child("User").child(auth.uid.toString()).child("collection").child("2").get()
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.action_favourite -> {
                if(isFavorite)
                    userDatabase.child("User").child(auth.uid.toString()).child("collection").child("2").setValue(false)
                    ...
                    userDatabase.child("User").child(auth.uid.toString()).child("collection").child("2").setValue(true)
                        .addOnSuccessListener {
                            item.setIcon(R.drawable.star)
                            isFavorite = true
